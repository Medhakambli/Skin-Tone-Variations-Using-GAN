...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Decode
        x8 = self.upsample(x7)
        # Ensure that x8 and x5 have the same size in dimension 1
        if x8.size() != x5.size():
        # Resize x8 so that it has the same size as x5
          x8 = torch.nn.functional.interpolate(x8, size=x5.size())

        # Concatenate x8 and x5 along dimension 1
        ...
        if x9.size() != x3.size():
        # Resize x8 so that it has the same size as x5
          x3 = torch.nn.functional.interpolate(x9, size=x3.size())
        ...
        if x10.size() != x1.size():
        # Resize x8 so that it has the same size as x5
          x10 = torch.nn.functional.interpolate(x10, size=x1.size())
